...
if (isset($_POST['delete_mes'])) {
    ...
    if (mysqli_affected_rows($conn) > 0) {
        $_SESSION['message'] = "A mes com o ID {$id_mes} foi excluída com sucesso!";
        $_SESSION['type'] = 'success';
    } else {
        $_SESSION['message'] = "Não foi possível excluir a mes.";
        $_SESSION['type'] = 'error';
    }
    header('Location: lista.php');
    exit;
...
if (isset($_POST['edit_mes'])) {
    ...
    if (mysqli_query($conn, $sql)) {
        if (mysqli_affected_rows($conn) > 0) {
            $_SESSION['message'] = 'Mês editada com sucesso!';
            $_SESSION['type'] = 'success'; 
        } else {
